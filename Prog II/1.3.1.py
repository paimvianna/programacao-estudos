...
class Filial:
    ...
    def info(self):
        ...
        #print(f'Livros em Estoque: R$ {self.livro.livros_em_estoque}')
       # print(f'Estoque: {self.Livro.quantidade_em_estoque}')
        print('')

    def cadastro_filial(self):
        codigo = int(input(f'Informe o numero da filial: '))
        codigo_filial = str(f'#FL{codigo}')
        ...
        self.contato_filial = contato_filial
        '''
        #self.lista_filiais.append(nova_filial)
        #self.livraria.acervo.append(nova_filial)
class Livraria:

    def __init__(self):
        self.acervo = []
        '''estou criando um atributo acervo, que vai ser uma lista vazia que irá
    ...
    def cadastro_livros (self):
        ...
        print(f'|______________________________________________________________________________|')
        print('')
        #self.filial.cadastro_filial() #chama o metodo de cadastro de filial da classe Filial
        self.acervo.append(self.filial.cadastro_filial())
        ...
        while continuar_cadastro:  # continuar variável boolena que se inicia com o true e no proxímo input e testado
            ...
            print(f'|______________________________________________________________________________|')
            print('')
            if len(self.filiais) == 0:
                nova_filial = self.filial.cadastro_filial()
                self.acervo.append(nova_filial)
                self.acervo.append(livro_novo)
                livro_novo.filial = self.filial
                self.filiais.append(self.filial)
            else:
                confirmacao_filial = input(f'Deseja mudar a filial. \nDigite SIM/S ou NÃO/N: ')
                if confirmacao_filial.lower() in ['s','y','sim','yes']:
                    nova_filial = self.filial.cadastro_filial()
                    self.acervo.append(nova_filial)
                    self.acervo.append(livro_novo)
    ...
    def gravar_acervo(self):
        ...
        for livro in self.acervo:
            ...
            if linha == f'{livro.codigo_filial},{nome_filial},{endereco_filial},{contato_filial}':
