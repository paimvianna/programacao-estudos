...
async function listarTodasConsultasNomeEspecialidade() {
    let todasConsultas = await CrudAPI.lerTodos();
    let especialidades = await CrudAPI.listaEspecialidades()
    let especialista
    todasConsultas.forEach(paciente => {
     especialista = especialidades.find(medico => medico.id == paciente.id_especialidade)   
    console.log(paciente.id+"-"+paciente.paciente+"-"+paciente.idade+"-"+especialista.nome);        
    });
...
async function buscarEspecialista() {
    ...
    let especialista = especialidades.find(medico => medico.id == especialidade)//usei o find pq quero o retorno do elemento objeto do arry no filter eu consigo um novo array com o elementos ou objetos.
    console.log("Lista de pacientes com o medico Especialista em: "+especialista.nome)
    //console.log(consultaEspecialidade)
    consultaEspecialidade.forEach((paciente) => console.log(paciente.id+"-"+paciente.paciente+"-"+paciente.idade+"-"+especialista.nome/*vou troca pelo nome do especialista paciente.id_especialidade*/))
}
async function buscarRgistrosProblema(){
    let listaPaciente = await CrudAPI.lerTodos();
    listaPaciente.forEach((paciente) => {
        if(paciente.idade >= 18 && paciente.id_especialidade){
    console.log(paciente.id+" - "+paciente.paciente+" - Idade: "+paciente.idade+" - Id-especialista "+paciente.id_especialidade);   
    }     
}
}
